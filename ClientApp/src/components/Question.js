...
const Question = () => {
    ...
    return (
        ...
        <Container className='' style={{ margin: 'auto' }}>
            <Container className="d-flex justify-content-center" style={{ paddingTop: '30px' }}>
                <ProgressBar now={now} label={`${now}%`} style={{ width: '80%', height: '3vh' }} />
                <h2 style={{ paddingLeft: '20px'}}>{questionIndex + 1} / 13</h2>
            </Container>
            ...
            <Container className="d-flex justify-content-center" style={{ marginTop: '20px' }}>
                ...
                {!mute ? (
                    ...
                    <h1 className="mt-3 mb-1 question" style={{ width:'980px', textAlign: 'center' }}>{cardsData[questionIndex].question}</h1>
                    <MdPlayCircle
                        onClick={togglePlay}
                        style={{
                            cursor: "pointer",
            ...
            <Container className="d-flex justify-content-center align-items-end">
                ...
                <Container className="arrow-container mt-5">
                    <MdArrowCircleLeft
                        style={{
                            cursor: "pointer",
                            fontSize: "4em",
            ...
            {![3, 4, 6, 7, 10, 11, 12].includes(questionIndex) && (
                <Container className="d-flex align-items-center justify-content-center">
                    <p style={{ marginTop:"-12px"}}>(You can select multiple answer choices if you'd like!)</p>
                </Container>
            )}
                    ...
                    <Row>
                        {cardsData[questionIndex].answers.map((card, idx) => (
                            <Col key={idx} xs={3} md={3} lg={3} style={{ marginBottom: '20px' }}>
                                <Button onClick={() => onPressAnswer(questionIndex, card.description)} className={`btn-answer ${getSelectedAnswers(questionIndex).includes(card.description) ? 'active' : ''}`} style={{ borderColor: 'white', backgroundColor: card.color, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', borderRadius: '20px', height:'370px' }} >
            ...
            {questionIndex < 11 && (
                <Container className=' d-flex justify-content-center'>
                    <Button
                        ...
                        style={{ borderRadius:'15px', borderColor:'#40bb85', backgroundColor: 'white' }}
                    >
                        <Card style={{ border: 'none' }}>
                            <Card.Title style={{ margin: 'auto', color: '#40bb85', fontWeight: 'bold' }}>I do not wish to answer.</Card.Title>
                        </Card>
                    </Button>
                </Container>
            )}
            ...
            <Modal show={showQuestionModal} onHide={closeQuestionModal} centered>
                ...
                <Modal.Footer>
                    <Button style={{fontWeight:"bold"}} variant="primary" onClick={closeQuestionModal}>Okay</Button>
                </Modal.Footer>
            </Modal>
